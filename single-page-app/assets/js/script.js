	...
	function render(url) {
		...
		var	map = {
			...
			'#add': function() {
				...
				renderAddComment(index, products);
			}
		...
		if(data.length){
			data.forEach(function (item) {
				if(item.id == index){
					...
					// AFICHER REVIEWS START
					$("#reviews").html('');
					if (item.reviews.length == 1) {
						$("#reviews").append('<h2>Reviews Utilisateurs</h2>')
						$("#reviews").append('<p>" '+item.reviews+' "</p>');
					}
					else {
						$("#reviews").append('<h2>Reviews Utilisateurs</h2>')
						var html = '<ul id="reviewslist">';
						for (i=0; i < item.reviews.length; i++) {
							html += '<li>" '+item.reviews[i]+' "</li>';
						}
						html += '</ul>';				
						$("#reviews").append(html);
					}
